...
fn get_text_surface(text: String) -> Surface<'static> {
    ...
    let font = match sdl2_tff_context.load_font(Path::new("src/JetBrainsMono-Regular.ttf"), 20) {
        Ok(font) => font,
...
fn render_font(
    ...
    let rendering_target = Rect::new(100, 100, width, height);
...
fn main() {
    ...
    let _ = render_font(&mut canvas, texture_creator);
